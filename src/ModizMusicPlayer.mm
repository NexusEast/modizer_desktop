...
#define MAX_USLEEP_WAIT 500*1000  //in usec
...
static int16_t *mac_sample_data;
static float *mac_sample_data_float;
static float *mac_sample_converted_data_float;
bool optMAC_loopmode = false;
int optMAC_loop_count = 2.0f;
int optMAC_resampleQuality=1;

static bool mMAC_force_loop;
static volatile int64_t mMAC_total_samples,mMAC_decode_pos_samples,mMAC_totalinternal_samples;
...
#define DEFAULT_PT3ZXTUNE 0
#define DEFAULT_PT3PT3P 1
...
#define DEFAULT_2SFXSF 0
#define DEFAULT_2SFVIO2SF 1
...
PlayerA vgm_player;
PlayerBase* vgm_plrEngine;
DATA_LOADER *vgm_loader;
WAVE_32BS vgm_renderBuffer[SOUND_BUFFER_SIZE_SAMPLE];
static UINT8 vgm_logLevel = DEVLOG_INFO;
static OS_MUTEX* vgm_renderMtx;    // render thread mutex
...
static const char* vgm_LogLevel2Str(UINT8 level)
{
...
static void vgm_PlayerLogCallback(void* userParam, PlayerBase* player, UINT8 level, UINT8 srcType,
...
static char vgmplay_activeChipsInstance[SOUND_VOICES_MAX_ACTIVE_CHIPS];
static const char *mod_activeChipsName[SOUND_VOICES_MAX_ACTIVE_CHIPS];
static uint8_t mod_activeChip_VoiceNb[SOUND_VOICES_MAX_ACTIVE_CHIPS];
static char mod_activeChipsNb;
...
//YOYOFR
static int vgm_getNoteFromFreq(double freq)
{
...
char *vgmGetVoiceDetailedVoiceName(UINT8 chipdId,UINT8 channel) {
    vgm_voice_name[0]=0;
    switch (chipdId) {
        case DEVID_SN76496: //SN76496
            snprintf(vgm_voice_name,32,"PSG %d",channel+1);
            break;
        case DEVID_YM2413: //YM2413
            snprintf(vgm_voice_name,32,"FM %d",channel+1);
            break;
        case DEVID_YM2612: //YM2612
            snprintf(vgm_voice_name,32,"FM %d",channel+1);
            break;
        case DEVID_YM2203: //YM2203
            if (channel<3) snprintf(vgm_voice_name,32,"FM %d",channel+1);
            else snprintf(vgm_voice_name,32,"SSG %d",channel-3+1);
            break;
        case DEVID_YM2608: //YM2608
            if (channel<6) snprintf(vgm_voice_name,32,"FM %d",channel+1);
            else if (channel<6+6) snprintf(vgm_voice_name,32,"Rhythm %d",channel-6+1);
            else if (channel<6+6+1) snprintf(vgm_voice_name,32,"ADPCM");
            else snprintf(vgm_voice_name,32,"SSG %d",channel-6-6-1+1);
            break;
        case DEVID_YM2610: //YM2610
            if (vgm2610b) {
                if (channel<6) snprintf(vgm_voice_name,32,"FM %d",channel+1);
                else if (channel<6+6) snprintf(vgm_voice_name,32,"ADPCM-A %d",channel-6+1);
                else if (channel<6+6+1) snprintf(vgm_voice_name,32,"ADPCM-B");
                else snprintf(vgm_voice_name,32,"SSG %d",channel-6-6-1+1);
            } else {
                if (channel<4) snprintf(vgm_voice_name,32,"FM %d",channel+1);
                else if (channel<4+6) snprintf(vgm_voice_name,32,"ADPCM-A %d",channel-4+1);
                else if (channel<4+6+1) snprintf(vgm_voice_name,32,"ADPCM-B");
...
-(id) initMusicPlayer {
    ...
    if ((self = [super init])) {
        ...
        snprintf(UADEscorename,PATH_MAX, "%s/score",UADEstate.config.basedir.name);
...
-(BOOL) iPhoneDrv_PlayRestart {
    ...
    for (i=0; i<SOUND_BUFFER_NB; i++) {
        ...
        for (int j=0;j<SOUND_BUFFER_SIZE_SAMPLE;j++) {
            ((int *)mBuffers[i]->mAudioData)[j]=last_audio_sample;
        }
...
int64_t src_callback_wsr(void *cb_data, float **data) {
    uint32_t howmany = SOUND_BUFFER_SIZE_SAMPLE;
    s_coreSwan->pUpdateWSR(hc_sample_data, howmany*4, howmany);
...
int64_t src_callback_mac(void *cb_data, float **data) {
    // render audio into sound buffer
    int16_t *sampleData;
    int64_t sampleGenerated;
    
    int nbSamplesToRender=mMAC_total_samples - mMAC_decode_pos_samples;
    if (mMAC_total_samples<0) {
    ...
    if (nbSamplesToRender<=0) {
        memset(mac_sample_data,0,SOUND_BUFFER_SIZE_SAMPLE*2*2);
        src_short_to_float_array (mac_sample_data, mac_sample_data_float,SOUND_BUFFER_SIZE_SAMPLE*2);
        *data=mac_sample_data_float;
        return SOUND_BUFFER_SIZE_SAMPLE;
    }
    
    sampleGenerated=nbSamplesToRender;
    
    int64_t real_available_samples=mMAC_totalinternal_samples-mMAC_decode_pos_samples;
    if (mMAC_total_samples<0) {
        real_available_samples=SOUND_BUFFER_SIZE_SAMPLE;
    }
    
    int channelsNb=(int)pAPEDecompress->GetInfo(APE_INFO_CHANNELS);
    short int *snd_ptr;
    int pBlocksRetrieved;
    sampleData=mac_sample_data;
    while (nbSamplesToRender) {
        if (nbSamplesToRender<=real_available_samples) {
            pAPEDecompress->GetData((char*)sampleData, nbSamplesToRender, &pBlocksRetrieved);
            mMAC_decode_pos_samples+=pBlocksRetrieved;
            nbSamplesToRender=0;
        } else {
            pAPEDecompress->GetData((char*)sampleData, real_available_samples, &pBlocksRetrieved);
            sampleData+=pBlocksRetrieved*channelsNb;
            mMAC_decode_pos_samples+=pBlocksRetrieved;
            nbSamplesToRender-=pBlocksRetrieved;
            //end reached, looping from start
            pAPEDecompress->Seek(0);
            mMAC_decode_pos_samples=0;
            real_available_samples=mMAC_totalinternal_samples-mMAC_decode_pos_samples;
        }
    }
    mCurrentSamples=mMAC_decode_pos_samples;
    
    if (channelsNb==1) {
        snd_ptr=mac_sample_data;
    ...
    src_short_to_float_array (mac_sample_data, mac_sample_data_float,sampleGenerated*2);
    *data=mac_sample_data_float;
                    ...
                    if (mPlayType==MMP_TIMIDITY) { //Special case : Timidity
                        ...
                        if (mNeedSeek==1) { //SEEK
                            ...
                            if (mPlayType==MMP_WSR) {//WSR
                                ...
                                if (mCurrentSamples > mSeekSamples) {
                                    if (m3uReader.size()) {
                                        s_coreSwan->pResetWSR(m3uReader[mod_currentsub].track);
                                    } else {
                                        s_coreSwan->pResetWSR(wsr_tracks[mod_currentsub-mod_minsub]);
                                    }
                                    mCurrentSamples=0;
                                }
                                mStartPosSamples=mCurrentSamples;
                                ...
                                while (mSeekSamples > mCurrentSamples) {
                                    ...
                                    int howmany=bufsize*(double)hc_sample_rate/(double)PLAYBACK_FREQ;
                                    s_coreSwan->pUpdateWSR(buffer_ana[buffer_ana_gen_ofs], howmany*4, howmany);
                                    mCurrentSamples += bufsize;
                                    iCurrentTime=mCurrentSamples*1000/PLAYBACK_FREQ;
                            ...
                            if (mPlayType==MMP_MAC) { //MAC
                                bGlobalSeekProgress=-1;
                                mMAC_decode_pos_samples=mNeedSeekTime*(int)pAPEDecompress->GetInfo(APE_INFO_SAMPLE_RATE)/1000;
                                mCurrentSamples=mMAC_decode_pos_samples;
                            ...
                            if (mPlayType==MMP_VGMPLAY) { //VGM
                                ...
                                if (!vgm_play_no_seek) {
                                    ...
                                    while (mSeekSamples - mCurrentSamples > 0) {
                                        ...
                                        nbBytes=vgm_player.Render(sample_to_skip*2*2,buffer_ana[buffer_ana_gen_ofs]);
                            ...
                            if (mPlayType==MMP_HC) { //HC
                                ...
                                //
                                while (mSeekSamples - mCurrentSamples > SOUND_BUFFER_SIZE_SAMPLE) {
                                    uint32_t howmany = SOUND_BUFFER_SIZE_SAMPLE*(double)hc_sample_rate/(double)PLAYBACK_FREQ;
                                ...
                                if (mSeekSamples - mCurrentSamples > 0)
                                {
                                    uint32_t howmany = (mSeekSamples - mCurrentSamples)*(double)hc_sample_rate/(double)PLAYBACK_FREQ;
                                    ...
                                    if (m_genNumVoicesChannels) {
                                        for (int j=0;j<(m_genNumVoicesChannels<SOUND_MAXVOICES_BUFFER_FX?m_genNumVoicesChannels:SOUND_MAXVOICES_BUFFER_FX);j++) {
                                            m_voice_prev_current_ptr[j]+=(mSeekSamples - mCurrentSamples)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                                            if ((m_voice_prev_current_ptr[j]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*2) m_voice_prev_current_ptr[j]=m_voice_prev_current_ptr[j]-((SOUND_BUFFER_SIZE_SAMPLE*2*4)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
                                        }
                                    }
                                    ...
                                    iCurrentTime=mCurrentSamples*1000/hc_sample_rate;
                                ...
                                if (m_genNumVoicesChannels&&(HC_type!=0x23)&&(HC_type!=0x41)&&(HC_type!=0x1)&&(HC_type!=0x2)) {
                                    for (int j=0;j<(m_genNumVoicesChannels<SOUND_MAXVOICES_BUFFER_FX?m_genNumVoicesChannels:SOUND_MAXVOICES_BUFFER_FX);j++) {
                                        memset(m_voice_buff[j],0,SOUND_BUFFER_SIZE_SAMPLE*4*4);
                                    }
                                }
                            ...
                            if (mPlayType==MMP_2SF) { //2SF
                                //                                bGlobalSeekProgress=-1;
                                //                                ds_seek_position(mNeedSeekTime);
                                //                                mCurrentSamples=mNeedSeekTime*PLAYBACK_FREQ/1000;
                                ...
                                while (mSeekSamples - mCurrentSamples > SOUND_BUFFER_SIZE_SAMPLE) {
                                    ...
                                    ds_read(buffer_ana[buffer_ana_gen_ofs],SOUND_BUFFER_SIZE_SAMPLE);
                                    mCurrentSamples+=SOUND_BUFFER_SIZE_SAMPLE;
                                    iCurrentTime=mCurrentSamples*1000/PLAYBACK_FREQ;
                            ...
                            if (mPlayType==MMP_KSS) {
                                if (m3uReader.size()) {
                                    ...
                                    if (iModuleLength<=0) {
                                        ...
                                        else iModuleLength=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;
                                    }
                                } else {
                                    KSSPLAY_reset(kssplay, mod_currentsub, 0);
                                    if (kss->info) {
                                ...
                                if (iModuleLength<0) iModuleLength=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;//SID_DEFAULT_LENGTH;
                                ...
                                if (iModuleLength<=0) iModuleLength=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;//SC68_DEFAULT_LENGTH;
                                ...
                                if (m3uReader.size()) {
                                    ...
                                    s_coreSwan->pResetWSR(wsr_tracks[mod_currentsub-mod_minsub]); //s_coreSwan->pGetFirstSong()
                                    s_coreSwan->pSetFrequency(48000);
                                    iModuleLength=0;
                                }
                                for (int j=0;j<m_genNumVoicesChannels;j++) {
                        ...
                        if (mPlayType==MMP_GME) {  //GME
                            ...
                            if (gme_track_ended(gme_emu)) {
                                ...
                                for (int j=0; j < m_genNumVoicesChannels; j++) {
                                    ...
                                    int vgmofs=(m_voice_prev_current_ptr[j]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)/(SOUND_BUFFER_SIZE_SAMPLE);
                                    vgmofs=vgmofs&15;
                                    vgmofs*=8;
                                ...
                                //MDZILog("ptr diff %d %d %d",diffptr,(int)m_voice_current_ptr[0],(int)m_voice_prev_current_ptr[0]);
                                ...
                                if (m_voicesDataAvail) {
                                    ...
                                    for (int j=0;j<(m_genNumVoicesChannels<SOUND_MAXVOICES_BUFFER_FX?m_genNumVoicesChannels:SOUND_MAXVOICES_BUFFER_FX);j++) {
                                        if (m_voice_fadeout_factor==256) {
                                            ...
                                            for (int i=0;i<SOUND_BUFFER_SIZE_SAMPLE;i++) {
                                                m_voice_buff_ana[buffer_ana_gen_ofs][i*SOUND_MAXVOICES_BUFFER_FX+j]=(m_voice_buff[j][(i+(m_voice_prev_current_ptr[j]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT))&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]*m_voice_fadeout_factor)>>8;
                                                m_voice_buff[j][(i+(m_voice_prev_current_ptr[j]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT))&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=0;
                                            }
                                        }
                                        m_voice_prev_current_ptr[j]+=SOUND_BUFFER_SIZE_SAMPLE<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                        ...
                        if (mPlayType==MMP_VGMPLAY) { //VGM
                            // render audio into sound buffer
                            if (vgm_plrEngine->GetState()&PLAYSTATE_END) {
                                nbBytes=0;
                            } else {
                                ...
                                if (m_genNumVoicesChannels) {
                                    for (int j=0;j<(m_genNumVoicesChannels<SOUND_MAXVOICES_BUFFER_FX?m_genNumVoicesChannels:SOUND_MAXVOICES_BUFFER_FX);j++) {
                                        ...
                                        memset(m_voice_buff_accumul_temp[j],0,SOUND_BUFFER_SIZE_SAMPLE*4*2);
                                        memset(m_voice_buff_accumul_temp_cnt[j],0,SOUND_BUFFER_SIZE_SAMPLE*2);
                                ...
                                if (m_genNumVoicesChannels) {
                                    for (int j=0;j<(m_genNumVoicesChannels<SOUND_MAXVOICES_BUFFER_FX?m_genNumVoicesChannels:SOUND_MAXVOICES_BUFFER_FX);j++)
                                        if (m_voicesWithDataAccmul[j]){
                                            for (int i=0;i<(m_voice_current_ptr[j]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT);i++) {
                                                int val=m_voice_buff_accumul_temp[j][i];
                                                int cnt=m_voice_buff_accumul_temp_cnt[j][i];
                                                if (cnt) val=val/cnt;
                                                m_voice_buff[j][i]=val;
                                            }
                                        }
                                }
                        ...
                        if (mPlayType==MMP_MAC) { //MAC
                            ...
                            nbBytes=src_callback_read (src_state,src_ratio,SOUND_BUFFER_SIZE_SAMPLE, mac_sample_converted_data_float)*2*2;
                            
                            src_float_to_short_array (mac_sample_converted_data_float,buffer_ana[buffer_ana_gen_ofs],SOUND_BUFFER_SIZE_SAMPLE*2) ;
                            
                            if (mMAC_total_samples>=0) {
                                if (mMAC_decode_pos_samples>=mMAC_total_samples) nbBytes=0;
                            }
                            mCurrentSamples=mMAC_decode_pos_samples;
                        ...
                        if (mPlayType==MMP_HC) { //Highly Complete
                            ...
                            if (iModuleLength>0) {
                                if (hc_fadeLength&&(hc_fadeStart>0)&&(hc_currentSample>hc_fadeStart)) {
                                    int startSmpl=hc_currentSample-hc_fadeStart;
                                    if (startSmpl>SOUND_BUFFER_SIZE_SAMPLE) startSmpl=SOUND_BUFFER_SIZE_SAMPLE;
                                    vol=256*(hc_fadeLength-(hc_currentSample-hc_fadeStart))/hc_fadeLength;
                                    if (vol<0) vol=0;
                                    if (vol>256) vol=256;
                                }
                            }
                            ...
                            if (m_genNumVoicesChannels) {
                                if ((HC_type==0x23)||(HC_type==0x21)||(HC_type==0x41)||(HC_type==0x1)||(HC_type==0x2)||(HC_type==0x11)||(HC_type==0x12))  { //SNSF, QSF, PSF, PSF2, SSF, DSF
                                    ...
                                    for (int j=0;j<(m_genNumVoicesChannels<SOUND_MAXVOICES_BUFFER_FX?m_genNumVoicesChannels:SOUND_MAXVOICES_BUFFER_FX);j++) {
                                        if (HC_type==0x21) {
                                            int newP=m_voice_current_ptr[j]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                                            int prevP=m_voice_prev_current_ptr[j]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                                            if (newP>=prevP) {
                                                if ((newP-prevP)<SOUND_BUFFER_SIZE_SAMPLE) {
                                                    m_voice_prev_current_ptr[j]=m_voice_current_ptr[j]-(SOUND_BUFFER_SIZE_SAMPLE<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
                                                    if (m_voice_prev_current_ptr[j]<0) {
                                                        m_voice_prev_current_ptr[j]+=(SOUND_BUFFER_SIZE_SAMPLE*4*2)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                                                    }
                                                }
                                            } else {
                                                newP+=(SOUND_BUFFER_SIZE_SAMPLE*4*2);
                                                if ((newP-prevP)<SOUND_BUFFER_SIZE_SAMPLE) {
                                                    m_voice_prev_current_ptr[j]=m_voice_current_ptr[j]-(SOUND_BUFFER_SIZE_SAMPLE<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
                                                    if (m_voice_prev_current_ptr[j]) {
                                                        m_voice_prev_current_ptr[j]+=(SOUND_BUFFER_SIZE_SAMPLE*4*2)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                                                    }
                                                }
                                            }
...
#define LIMIT16(a) (a<-32768?-32768:(a>32767?32767:a))
                        ...
                        if (mPlayType==MMP_STSOUND) { //STSOUND
                            ...
                            if (ymMusicComputeStereo((void*)ymMusic,(ymsample*)buffer_ana[buffer_ana_gen_ofs],nbSample)==YMTRUE) {
                                nbBytes=SOUND_BUFFER_SIZE_SAMPLE*2*2;
                                mCurrentSamples+=SOUND_BUFFER_SIZE_SAMPLE;
                            } else {
                                nbBytes=0;
                        ...
                        if (mPlayType==MMP_PIXEL) {
                            if (pixel_organya_mode) {
                                ...
                                if (!org_gensamples((char*)(buffer_ana[buffer_ana_gen_ofs]), SOUND_BUFFER_SIZE_SAMPLE)) {
                                    ...
                                    memset(tim_notes[buffer_ana_gen_ofs],0,DEFAULT_VOICES*4);
                                    for (int j=0; j < m_genNumVoicesChannels; j++) {
                                        if (m_voicesStatus[j]&&vgm_last_vol[j]) {
                                            unsigned int idx=vgm_getNote(j);
                                            if ((idx>0)) {
                                                unsigned int subidx=vgm_getSubNote(j);
                                                // MDZILog("ch %d note %d vol %d\n",j,idx,vgm_last_vol[j]);
                        ...
                        if (mPlayType==MMP_NSFPLAY) {
                            ...
                            for(int i = 0; i < 2; i++) {
                                ...
                                vgm_last_note[i]=info->GetFreqHz()*2;
...
-(int) getSongLengthfromMD5:(int)track_nb {
    ...
    if (songlength==-1) {
        ...
        if (sqlite3_open([pathToDB UTF8String], &db) == SQLITE_OK){
            ...
            snprintf(sqlStatement,1024,"SELECT song_length FROM songlength_user WHERE id_md5=\"%s\" AND track_nb=%d",song_md5,track_nb);
            err=sqlite3_prepare_v2(db, sqlStatement, -1, &stmt, NULL);
            if (err==SQLITE_OK){
                while (sqlite3_step(stmt) == SQLITE_ROW) {
                    songlength=sqlite3_column_int(stmt, 0)*1000;
...
-(void) getStilInfo:(char*)fullPath {
    ...
    if (sqlite3_open([pathToDB UTF8String], &db) == SQLITE_OK){
        ...
        if (!realPath) {
            ...
            snprintf(sqlStatement,1024,"SELECT filepath FROM hvsc_path WHERE id_md5=\"%s\"",song_md5);
            err=sqlite3_prepare_v2(db, sqlStatement, -1, &stmt, NULL);
            if (err==SQLITE_OK){
                while (sqlite3_step(stmt) == SQLITE_ROW) {
...
-(void) getStilAsmaInfo:(char*)fullPath {
    ...
    if (sqlite3_open([pathToDB UTF8String], &db) == SQLITE_OK){
        ...
        if (!realPath) {
            //try to find realPath with md5
            snprintf(sqlStatement,1024,"SELECT filepath FROM asma_path WHERE id_md5=\"%s\"",song_md5);
            err=sqlite3_prepare_v2(db, sqlStatement, -1, &stmt, NULL);
            if (err==SQLITE_OK){
                while (sqlite3_step(stmt) == SQLITE_ROW) {
                    strcpy(tmppath,(const char*)sqlite3_column_text(stmt, 0));
...
-(int) mmp_gsfLoad:(NSString*)filePath {  //GSF
    ...
    silencelength=5;
    IgnoreTrackLength=0;
    DefaultLength=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;
    ...
    if (!res) {
        ...
        if (gsf_libfile) {
            //missing lib
            snprintf(mplayer_error_msg,1024,"Missing GSFLIB: %s",gsf_libfile);
        }
        return -1;
    }
...
-(int) mmp_mdxpdxLoad:(NSString*)filePath {  //MDX
    ...
    if (mdx_load((char*)[filePath UTF8String],&mdx,&pdx,(mLoopMode==1?32767:settings[GLOB_Maxloop].detail.mdz_slider.slider_value)) ) {
        ...
        if (iModuleLength<=0) iModuleLength=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;//MDX_DEFAULT_LENGTH;
        iCurrentTime=0;
        mCurrentSamples=0;
        ...
        if (tmp_mod_name) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"Title.....: %s\n",tmp_mod_name);
        else snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"Title.....: N/A\n");
...
-(int) mmp_sc68Load:(NSString*)filePath {  //SC68
    ...
    if (sc68_load_uri(sc68,[filePath UTF8String])) {
        ...
        for (int i=0;i<mod_subsongs; i++) {
            ...
            if (subsong_length<=0) subsong_length=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;//SC68_DEFAULT_LENGTH;
            mod_total_length+=subsong_length;
...
static WSRPlayerApi* s_coreSwan=&oswan::g_wsr_player_api;

- (int) mmp_wsrLoad:(NSString*)filePath {  //WSR
    mPlayType=MMP_WSR;
    ...
    wsr_music_size=mp_datasize;
    wsr_music_buf=(char*)malloc(mp_datasize+1);
    ...
    fread(wsr_music_buf,1,wsr_music_size,f);
    wsr_music_buf[wsr_music_size]=0;
    fclose(f);
        ...
        for (uint32_t i = 0; i < 256; i++)
        {
            ...
            while (numSamples)
            {
                ...
                for (uint32_t j = 0; j < 512; j++)
                {
                    if (buf[j] != smp)
                    {
                        wsr_tracks[numSubsongs] = uint8_t(i);
                        numSubsongs++;
                        numSamples = 0;
                        break;
                    }
    ...
    if (m3uReader.info().artist)
        if (m3uReader.info().artist[0]) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,  "%sArtist..: %s\n",mod_message,m3uReader.info().artist);
    if (m3uReader.info().composer)
        if (m3uReader.info().composer[0]) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%sComposer: %s\n",mod_message,m3uReader.info().composer);
    if (m3uReader.info().date)
        if (m3uReader.info().date[0]) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,    "%sDate....: %s\n",mod_message,m3uReader.info().date);
    if (m3uReader.info().ripping)
        if (m3uReader.info().ripping[0]) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2, "%sRipping.: %s\n",mod_message,m3uReader.info().ripping);
    if (m3uReader.info().tagging)
        if (m3uReader.info().tagging[0]) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2, "%sTagging.: %s\n",mod_message,m3uReader.info().tagging);
    ...
    numChannels=6; //4channels + voice + noise
    m_voicesDataAvail=1;
    m_genNumVoicesChannels=numChannels;
    for (int i=0;i<m_genNumVoicesChannels;i++) {
    ...
    if (mod_subsongs>1) {
        ...
        for (int i=0;i<mod_subsongs;i++) {
            ...
            if (subsong_length<=0) {
                subsong_length=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;
            }
...
-(int) mmp_pixelLoad:(NSString*)filePath {  //PxTone Collage & Organya
    ...
    FILE *f=fopen([filePath UTF8String],"rb");
    if (f==NULL) {
        MDZELog("Pixel Cannot open file %@",filePath);
        mPlayType=0;
        return -1;
    }
...
-(int) mmp_nezLoad:(NSString*)filePath {  //Game Gear & PCEngine
    ...
    FILE *f=fopen([filePath UTF8String],"rb");
    if (f==NULL) {
        MDZELog("NEZ Cannot open file %@",filePath);
        mPlayType=0;
        return -1;
    }
    ...
    fseek(f,0L,SEEK_END);
    mp_datasize=ftell(f);
    nez_fileBufferLen=mp_datasize;
    nez_fileBuffer=(char*)malloc(nez_fileBufferLen);
    if (!nez_fileBuffer) {
        MDZELog("cannot allocate nez_fileBuffer");
        fclose(f);
        return -1;
    }
    fseek(f,0L,SEEK_SET);
    ...
    _nezPlay = NEZNew();
    if (!_nezPlay) {
        MDZELog("cannot allocate _nezPlay");
...
-(int) mmp_macLoad:(NSString*)filePath {  //MAC Monkey Audio Codec
    ...
    FILE *f=fopen([filePath UTF8String],"rb");
    if (f==NULL) {
        MDZELog("MAC Cannot open file %@",filePath);
        mPlayType=0;
        return -1;
    }
    ...
    //Initi SRC samplerate converter
    int error;
    src_state=src_callback_new(src_callback_mac,optMAC_resampleQuality,2,&error,NULL);
    if (!src_state) {
        MDZELog("Error while initializing SRC samplerate converter: %d",error);
        return -1;
    }
    
    if (mLoopMode==1) mMAC_force_loop = true;
    else mMAC_force_loop = false;
    ...
    {
        MDZELog("Error opening APE file. (error code %d)\r\n\r\n", nRetVal);
        src_delete(src_state);
        return -1;
    }
    ...
    snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%sAPE version: %.2f\r\n",mod_message,float(pAPEDecompress->GetInfo(APE_INFO_FILE_VERSION)) / 1000.0);
    switch (pAPEDecompress->GetInfo(APE_INFO_COMPRESSION_LEVEL))
    {
        case COMPRESSION_LEVEL_FAST: snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\tCompression level: Fast\r\n\r\n",mod_message); break;
        case COMPRESSION_LEVEL_NORMAL: snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\tCompression level: Normal\r\n\r\n",mod_message); break;
    ...
    snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\tBits per sample: %d\r\n",mod_message, (int)pAPEDecompress->GetInfo(APE_INFO_BITS_PER_SAMPLE));
    ...
    if (mLoopMode==1) {
        ...
        mMAC_total_samples = (long)pAPEDecompress->GetInfo(APE_INFO_LENGTH_MS) * PLAYBACK_FREQ / 1000;
        mMAC_totalinternal_samples = mMAC_total_samples;
    }
    mMAC_decode_pos_samples=0;
    ...
    iModuleLength=(int)pAPEDecompress->GetInfo(APE_INFO_LENGTH_MS);
    iCurrentTime=0;
    mCurrentSamples=0;
    ...
    // tag information
    snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%sTag Information:\r\n",mod_message);
    ...
    {
        snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\tID3 Tag: %s, APE Tag: %s",mod_message, bHasID3Tag ? "Yes" : "No", bHasAPETag ? "" : "No");
        if (bHasAPETag)
        {
            snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s%d",mod_message, pAPETag->GetAPETagVersion() / 1000);
        }
        snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\n\n",mod_message);
        ...
        while ((pTagField = pAPETag->GetTagField(index)) != NULL)
        {
            ...
            {
                snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\t%s: --- too much data to display ---\r\n",mod_message, CAPECharacterHelper::GetANSIFromUTF16(pTagField->GetFieldName()));
            }
            else
            {
                /*
    ...
    {
        snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\tNot tagged\r\n\r\n",mod_message);
    }
...
-(int) mmp_websidLoad:(NSString*)filePath {  //SID
    ...
    if (!result) {
        ...
        m_freqTable = (is_ntsc ? freqTableNtsc : freqTablePal);
        ...
        for (int i=0;i<mod_subsongs; i++) {
            ...
            if (sid_subsong_length<0) sid_subsong_length=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;//SID_DEFAULT_LENGTH;
        ...
        snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"");
        snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"Chip: %s\n",(SID::isSID6581()?"6581":"8580"));
...
-(int) mmp_sidplayLoad:(NSString*)filePath {  //SID
    ...
    if (mSidTune==NULL) {
        ...
        if (mSidEmuEngine->load(mSidTune)) {
            ...
            if (cfg.forceSidModel) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"Chip: %s\n",(cfg.defaultSidModel==SidConfig::MOS8580?"8580":"6581"));
            else snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"Chip: %s\n",(sidtune_info->sidModel(0)==SidTuneInfo::SIDMODEL_8580?"8580":"6581"));
    ...
    if (mod_subsongs>1) {
        ...
        for (int i=0;i<mod_subsongs;i++) {
            ...
            if (subsong_length<=0) {
                ...
                else subsong_length=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;
            }
...
-(int) mmp_hvlLoad:(NSString*)filePath {  //HVL
    ...
    if (hvl_song==NULL) {
        ...
        if (hvl_song->ht_InstrumentNr==0) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"N/A\n");
        else {
            snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"");
...
-(int) mmp_zxtuneLoad:(NSString*)filePath {  //HVL
    ...
    snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"Title..: %s\nAuthor.: %s\nComment: %s\nProgram: %s\nSubpath: %s\n",
             zxtune_song_info.get_title(),
             zxtune_song_info.get_author(),
             zxtune_song_info.get_comment(),
             zxtune_song_info.get_program(),
             zxtune_song_info.get_subpath());
...
-(int) mmp_uadeLoad:(NSString*)filePath {  //UADE
    ...
    if (strcmp(UADEstate.ep->playername, "custom") == 0) {
        ...
        snprintf(UADEplayername,1024,"%s/%s", UADEstate.config.basedir.name, UADEstate.ep->playername);
    }
...
-(int) mmp_xmpLoad:(NSString*)filePath {  //XMP
    ...
    if (xmp_mi->comment) snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"%s\n", xmp_mi->comment);
    else {
        mod_message[0]=0;
        if (xmp_mi->mod->ins) {
...
-(int) mmp_timidityLoad:(NSString*)filePath { //timidity
    ...
    snprintf(mod_name,sizeof(mod_name)," %s",mod_filename);
    snprintf(mod_message,MAX_STIL_DATA_LENGTH*2,"Midi Infos:");
...
void ds_meta_set(const char * tag, const char * value) {
    ...
    if (mod_message[0]) concatn(MAX_STIL_DATA_LENGTH*2,mod_message,"\n");
    concatn(MAX_STIL_DATA_LENGTH*2,mod_message,tag);
    concatn(MAX_STIL_DATA_LENGTH*2,mod_message,"=");
    concatn(MAX_STIL_DATA_LENGTH*2,mod_message,value);
    
    // propagate selected meta info for use in GUI
    if (!strcasecmp(tag, "title")) {
        //info->setText(0, value, "");
        mod_title=[NSString stringWithUTF8String:value];
    } else if (!strcasecmp(tag, "length")) {
        int min,sec;
        sscanf(value,"%d:%d",&min,&sec);
        iModuleLength=(min*60+sec)*1000;
    } else if (!strcasecmp(tag, "artist"))
    {
        //        info->setText(1, value, "");
        mod_artist=[NSString stringWithUTF8String:value];
    } else  if (!strcasecmp(tag, "fade")) {
        sscanf(value,"%d",&mod_fadeinMs);
        mod_fadeinMs*=1000;
    } else if (!strcasecmp(tag, "album"))
    {
        //        info->setText(2, value, "");
        snprintf(mod_name,sizeof(mod_name)," %s",value);
    }
...
-(int) mmp_ncsfLoad:(NSString*)filePath extension:(NSString*)extension {  //NCSF
    mPlayType=MMP_NCSF;
    FILE *f;
    ...
    if (([extension caseInsensitiveCompare:@"NCSF"]==NSOrderedSame)||
        ([extension caseInsensitiveCompare:@"MININCSF"]==NSOrderedSame)) {
        xSFPlayer=new XSFPlayer_NCSF([filePath UTF8String]);
        xsf_mode=0;
    }
    
    if (([extension caseInsensitiveCompare:@"2SF"]==NSOrderedSame)||
        ([extension caseInsensitiveCompare:@"MINI2SF"]==NSOrderedSame)) {
        xSFPlayer=new XSFPlayer_2SF([filePath UTF8String]);
        xsf_mode=1;
    }
...
-(int) mmp_HCLoad:(NSString*)filePath {  //Highly Complete
    ...
    if (!info.tag_length_ms) {
        info.tag_length_ms = settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;
...
-(int) mmp_vgmplayLoad:(NSString*)filePath { //VGM
    ...
    vgm_player.RegisterPlayerEngine(new VGMPlayer);
    vgm_player.RegisterPlayerEngine(new S98Player);
    vgm_player.RegisterPlayerEngine(new DROPlayer);
    vgm_player.RegisterPlayerEngine(new GYMPlayer);
    //vgm_player.SetEventCallback(FilePlayCallback, NULL);
    vgm_player.SetFileReqCallback(vgm_RequestFileCallback, NULL);
    vgm_player.SetLogCallback(vgm_PlayerLogCallback, NULL);
    
    /* setup the player's output parameters and allocate internal buffers */
    if (vgm_player.SetOutputSettings(PLAYBACK_FREQ, 2, 16, SOUND_BUFFER_SIZE_SAMPLE)) {
    ...
    if (OSMutex_Init(&vgm_renderMtx, 0)) {
        vgm_player.UnloadFile();
        DataLoader_Deinit(vgm_loader);
        vgm_player.UnregisterAllPlayers();
        vgm_loader=NULL;
        return 1;
    ...
    vgm_plrEngine = vgm_player.GetPlayer();
    ...
    vgm_plrEngine->GetSongInfo(sInf);
    vgm_plrEngine->GetSongDeviceInfo(diList);
    
    vgm_play_no_seek=false;
    
    char firstChip=1;
    for (curDev = 0; curDev < diList.size(); curDev ++) {
        const PLR_DEV_INFO& pdi = diList[curDev];
        
        if (pdi.instance==0xFF) continue;
        ...
        if (pdi.type==DEVID_YM2610) {
            ...
            vgm_play_no_seek=true; //vgm native seek doesn't work for ADPCM part
        }
        vgmplay_activeChips[mod_activeChipsNb]=pdi.type;
        vgmplay_activeChipsID[mod_activeChipsNb]=pdi.id;
        vgmplay_activeChipsInstance[mod_activeChipsNb]=pdi.instance;
        m_voice_ChipID[numChannels]=pdi.id;
        
        mod_activeChipsName[mod_activeChipsNb]=SndEmu_GetDevName(pdi.type, 0x01, pdi.devCfg);
        numChannels+=vgmGetVoicesNb(pdi.type);
        
        for (int j=m_genNumVoicesChannels;j<m_genNumVoicesChannels+vgmGetVoicesNb(pdi.type);j++) {
            m_voice_voiceColor[j]=m_voice_systemColor[mod_activeChipsNb];
        ...
        if (firstChip) {
            snprintf(mod_message+strlen(mod_message),MAX_STIL_DATA_LENGTH*2,"Chipsets....: %s",mod_activeChipsName[mod_activeChipsNb]);
            firstChip=0;
        } else snprintf(mod_message+strlen(mod_message),MAX_STIL_DATA_LENGTH*2,",%s",mod_activeChipsName[mod_activeChipsNb]);
        
        
        mod_activeChipsNb++;
    }
    
    if (m_genNumVoicesChannels) m_voicesDataAvail=1;
    
    std::vector<std::string> langPostfixes;
    int defaultLang = settings[VGMPLAY_PreferJTAG].detail.mdz_boolswitch.switch_value ? 1 : 0;
    ...
    if (tagList != NULL) {
        ...
        snprintf(mod_message+strlen(mod_message),MAX_STIL_DATA_LENGTH*2-strlen(mod_message),"\nAuthor......: %s\nGame........: %s\nSystem......: %s\nTitle.......: %s\nRelease Date: %s\nCreator.....: %s\nNotes.......: %s\n",
                 songAuthor,songGame,songSystem,songTitle,songDate,songEncoder,songComment);
        ...
        artist=[NSString stringWithUTF8String:songAuthor];
        album=[NSString stringWithUTF8String:songGame];
        ...
        mod_title=[NSString stringWithUTF8String:songTitle];
        if (mod_title && ([mod_title length]==0)) mod_title=nil;
        
        snprintf(mod_name,sizeof(mod_name)," %s",songGame);
...
-(int) mmp_pmdminiLoad:(NSString*)filePath {
    ...
    if (!pmd_is_pmd((char*)[filePath UTF8String])) {
        ...
        if (iModuleLength<=0) iModuleLength=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;//PMD_DEFAULT_LENGTH;
        iCurrentTime=0;
        mCurrentSamples=0;
        ...
        memset(mod_activeChip_VoiceNb,0,sizeof(mod_activeChip_VoiceNb));
        int idx=0;
        mod_activeChipsNb=0;
        if (pmd_system_voice_nb[0]>0) { //FM
            for (int i=idx;i<idx+pmd_system_voice_nb[0];i++) {
                m_voice_voiceColor[i]=m_voice_systemColor[mod_activeChipsNb];
            }
            mod_activeChipsName[mod_activeChipsNb]="FM";
            mod_activeChip_VoiceNb[mod_activeChipsNb]=pmd_system_voice_nb[0];
            idx+=pmd_system_voice_nb[0];
            mod_activeChipsNb++;
        }
        if (pmd_system_voice_nb[1]>0) { //SSG
            for (int i=idx;i<idx+pmd_system_voice_nb[1];i++) {
                m_voice_voiceColor[i]=m_voice_systemColor[mod_activeChipsNb];
            }
            mod_activeChipsName[mod_activeChipsNb]="SSG";
            mod_activeChip_VoiceNb[mod_activeChipsNb]=pmd_system_voice_nb[1];
            idx+=pmd_system_voice_nb[1];
            mod_activeChipsNb++;
        }
        if (pmd_system_voice_nb[2]>0) { //PPZ
            for (int i=idx;i<idx+pmd_system_voice_nb[2];i++) {
                m_voice_voiceColor[i]=m_voice_systemColor[mod_activeChipsNb];
            }
            mod_activeChipsName[mod_activeChipsNb]="PPZ";
            mod_activeChip_VoiceNb[mod_activeChipsNb]=pmd_system_voice_nb[2];
            idx+=pmd_system_voice_nb[2];
            mod_activeChipsNb++;
        }
    ...
    if (err) {
        ...
        for (int i=mod_minsub;i<=mod_maxsub; i++) {
            if (gme_track_info( gme_emu, &gme_info, i )==0) {
                ...
                if (m3uPlayLength) gme_subsong_length=m3uPlayLength;
                
                if (gme_subsong_length<=0) gme_info->play_length=settings[GLOB_DefaultLength].detail.mdz_slider.slider_value*1000;
        ...
        if (gme_track_info( gme_emu, &gme_info, mod_currentsub )==0) {
            iModuleLength=gme_info->play_length;
            if (m3uPlayLength) iModuleLength=m3uPlayLength;
            strcpy(gmetype,gme_info->system);
...
-(int) LoadModule:(NSString*)_filePath archiveMode:(int)archiveMode archiveIndex:(int)archiveIndex singleSubMode:(int)singleSubMode singleArcMode:(int)singleArcMode detailVC:(DetailViewControllerIphone*)detailVC isRestarting:(bool)isRestarting shuffle:(bool)shuffle{
    ...
    NSArray *filetype_extZXTUNE=[SUPPORTED_FILETYPE_ZXTUNE componentsSeparatedByString:@","];
    ...
    mdz_default2SFPLAYER=settings[GLOB_Default2SFPlayer].detail.mdz_switch.switch_value;
    ...
    if (archiveMode==0) {
        ...
        snprintf(mod_filename,1024,"%s",[[[filePath lastPathComponent] stringByDeletingPathExtension] UTF8String]);
        ...
        if (found) { //might be an archived file
            ...
            if (found==1) { //Archive
                ...
                if (mdz_ArchiveFilesCnt) {
                    ...
                    if ((found==1)&&(!skip_extract)&&(!isRestarting)) { //Archive
                    ...
                    last_archive_filepath=[NSString stringWithString:_filePath];
...
//                            }
                            ...
                            for (int i=0;i<mdz_ArchiveFilesCnt;i++) {
                                ...
                                if (m3uReader[i].track>=0) {
                                    ...
                                    if (m3uReader[i].decimal_track && (m3u_minEntry>0)&&(strcmp(m3uReader[i].type,"NSF")==0) ) mdz_ArchiveEntryMonoSub[i]--;
                                }
...
//                                    }
                                    ...
                                    if (songlength==0) {
                                        songlength=int([SPCTagParser ticksToSeconds:tag.introLength]*1000);
                                    }
    ...
    for (int i=0;i<[filetype_extMAC count];i++) {
        if ([extension caseInsensitiveCompare:[filetype_extMAC objectAtIndex:i]]==NSOrderedSame) {
            [available_player addObject:[NSNumber numberWithInt:MMP_MAC]];
            break;
        }
    }
    ...
    for (int i=0;i<[filetype_extPT3 count];i++) {
        if ([extension caseInsensitiveCompare:[filetype_extPT3 objectAtIndex:i]]==NSOrderedSame) {
            if (mdz_defaultPT3PLAYER==DEFAULT_PT3PT3P) [available_player insertObject:[NSNumber numberWithInt:MMP_PT3] atIndex:0];
    ...
    for (int i=0;i<[available_player count];i++) {
        ...
        switch (pl_idx) {
            ...
            case MMP_2SF:
                ...
                break;
            case MMP_NCSF:
                retval=[self mmp_ncsfLoad:filePath extension:extension];
                break;
            case MMP_V2M:
            ...
            case MMP_PIXEL:
                ...
                break;
            case MMP_WSR:
                retval=[self mmp_wsrLoad:filePath];
                break;
            case MMP_ZXTUNE:
                retval=[self mmp_zxtuneLoad:filePath];
                break;
            case MMP_EUP:
...
-(void) Stop {
    ...
    if (mPlayType==MMP_TIMIDITY) { //Timidity
        ...
        while (!tim_finished) {
            [NSThread sleepForTimeInterval:DEFAULT_WAIT_TIME_MS];
            mod_message_updated=0; //avoid deadlock if waiting for the same in generate sound thread
        }
    }
    ...
    if (mPlayType==MMP_WSR) {
        s_coreSwan->pCloseWSR();
        mdz_safe_free(hc_sample_data);
        mdz_safe_free(hc_sample_data_float);
        mdz_safe_free(hc_sample_converted_data_float);
        mdz_safe_free(wsr_music_buf);
        if (src_state) src_delete(src_state);
        src_state=NULL;
    }
...
//            MDZILog("sid: load NULL");
    ...
    if (mPlayType==MMP_VGMPLAY) { //VGM
        ...
        vgm_player.UnregisterAllPlayers();
        if (vgm_loader) DataLoader_Deinit(vgm_loader);
        vgm_loader=NULL;
    }
    if (mPlayType==MMP_2SF) { //2SF
    }
...
-(NSString*) getSubTitle:(int)subsong {
    ...
    if (mPlayType==MMP_VGMSTREAM) {
        ...
        return result;
    } else if (mPlayType==MMP_ZXTUNE) {
        if (mdz_zxtune) {
            zxtune_song_info.reset();
            mdz_zxtune->get_song_info(subsong+1,zxtune_song_info);
            const char *title=zxtune_song_info.get_title();
            if (title && title[0]) {
                return [[NSString stringWithFormat:@"%.3d-%@",subsong-mod_minsub+1,[NSString stringWithCString:title encoding:NSShiftJISStringEncoding]]  stringByReplacingOccurrencesOfString:@"\"" withString:@"'"];
            }
        }
...
-(NSString*) getSystemsName:(int)systemIdx {
    switch (mPlayType) {
        ...
        case MMP_HC:
            ...
            else if (HC_type==0x21) return @"N64";
...
-(int) getSystemForVoice:(int)voiceIdx {
    switch (mPlayType) {
        ...
        case MMP_PMDMINI: {
            int idx=0;
            for (int i=0;i<mod_activeChipsNb;i++) {
                if (voiceIdx<idx+mod_activeChip_VoiceNb[i]) return i;
                idx+=mod_activeChip_VoiceNb[i];
            }
            return 0;
        }
...
-(int) getSystemm_voicesStatus:(int)systemIdx {
    ...
    switch (mPlayType) {
        ...
        case MMP_HC:
            ...
            } else if ((HC_type==0x11)||(HC_type==0x12)||(HC_type==0x21)||(HC_type==0x23)||(HC_type==0x41)) { //scsp or aica or n64 or spc700 or qsound
                for (int i=0;i<numChannels;i++) tmp+=(m_voicesStatus[i]?1:0);
                if (tmp==numChannels) return 2; //all active
                else if (tmp>0) return 1; //partially active
            }
        ...
        case MMP_PMDMINI: {
            ...
            tmp=0;
            for (int i=0;i<mod_activeChip_VoiceNb[systemIdx];i++) {
                tmp+=(m_voicesStatus[idx+i]?1:0);
            }
            if (tmp==mod_activeChip_VoiceNb[systemIdx]) return 2; //all active
            else if (tmp>0) return 1; //partially active
            return 0; //all off
        }
        case MMP_VGMPLAY: {
...
-(void) setSystemm_voicesStatus:(int)systemIdx active:(bool)active {
    switch (mPlayType) {
        case MMP_HC:
            ...
            else if (HC_type==0x21) for (int i=0;i<numChannels;i++) [self setm_voicesStatus:active index:i]; //N64
...
-(void) setm_voicesStatus:(bool)active index:(unsigned int)channel {
    ...
    switch (mPlayType) {
        ...
        case MMP_WSR:
            for (int i=0;i<m_genNumVoicesChannels;i++) if (!(m_voicesStatus[i])) new_mutemask|=1<<i;
            s_coreSwan->pSetChannelMuting(new_mutemask);
            break;
        case MMP_NCSF:
        ...
        case MMP_VGMPLAY:{
            ...
            for (int i=0;i<mod_activeChipsNb;i++) {
                if ((channel>=idx)&&(channel<idx+vgmGetVoicesNb(vgmplay_activeChips[i]))) {
                    ...
                    vgm_plrEngine->GetDeviceMuting(vgmplay_activeChipsID[i], muteOpts);
                    
                    int chip_channel=channel-idx;
                    ...
                    switch (vgmplay_activeChips[i]) {
                        ...
                        case 1: //VRC7 or YM2413
                            ...
                            /* 9->6, 10/11->7, 12/13->8*/
                            /*
                             EOPLL_MASK_BD, EOPLL_MASK_SD, EOPLL_MASK_TOM, EOPLL_MASK_CYM, EOPLL_MASK_HH
                             */
                            
                            if (chip_channel==6) {
                                //BD
                                if (active) muteOpts.chnMute[0]&=~(1<<9);
                                else muteOpts.chnMute[0]|=1<<9;
                            } else if (chip_channel==7) {
                                //HH & SD
                                if (active) {
                                    muteOpts.chnMute[0]&=~(1<<13);
                                    muteOpts.chnMute[0]&=~(1<<10);
                                } else {
                                    muteOpts.chnMute[0]|=1<<13;
                                    muteOpts.chnMute[0]|=1<<10;
                                }
                                ...
                                //TOM & CYM
                                if (active) {
                                    muteOpts.chnMute[0]&=~(1<<11);
                                    muteOpts.chnMute[0]&=~(1<<12);
                                } else {
                                    muteOpts.chnMute[0]|=1<<11;
                                    muteOpts.chnMute[0]|=1<<12;
                                }
                        ...
                        case DEVID_YM2612: //YM2612:  6voices
                            ...
                            if (chip_channel==5) {
                                ...
                                if (active) muteOpts.chnMute[0]&=~(1<<6);
                        ...
                        case DEVID_YM2610://YM2610: 14voices (4 fm), YM2610b: 16voices (6 fm)
                            ...
                            if (vgm2610b==0) {
                                if (chip_channel<4) {
                                    switch (chip_channel) {
                                        case 0:chip_channel=1;break;
                                        case 1:chip_channel=2;break;
                                        case 2:chip_channel=4;break;
                                        case 3:chip_channel=5;break;
                                    }
                                } else chip_channel+=2; //adpcm start at 6
                            }
                            
                            if (chip_channel<13) { //FM&ADPCM
                                if (active) muteOpts.chnMute[0]&=~(1<<chip_channel);
                                else muteOpts.chnMute[0]|=(1<<chip_channel);
                    ...
                    vgm_plrEngine->SetDeviceMuting(vgmplay_activeChipsID[i], muteOpts);
